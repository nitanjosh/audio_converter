...
.main-title { font-family: 'Space Grotesk', sans-serif; font-size: 2.8rem; font-weight: 700; letter-spacing: -1px; background: linear-gradient(135deg, #f0ede6 0%, #c8b89a 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 0.2rem; width: fit-content; }
.sub-title { font-family: 'DM Mono', monospace; font-size: 0.78rem; color: #666; letter-spacing: 3px; text-transform: uppercase; margin-bottom: 2rem; }
.file-card { background: #161616; border: 1px solid #2a2a2a; border-radius: 10px; padding: 12px 16px; margin: 6px 0; display: flex; justify-content: space-between; align-items: center; font-size: 0.82rem; transition: border-color 0.2s; }
...
.section-label { font-family: 'DM Mono', monospace; font-size: 0.68rem; letter-spacing: 3px; text-transform: uppercase; color: #444; margin: 1.4rem 0 0.6rem; }
hr { border-color: #1f1f1f !important; margin: 1.5rem 0 !important; }
.stButton > button { background: #f0ede6 !important; color: #0d0d0d !important; font-family: 'Space Grotesk', sans-serif !important; font-weight: 600 !important; font-size: 0.85rem !important; letter-spacing: 1px !important; border: none !important; border-radius: 8px !important; padding: 0.55rem 1.6rem !important; transition: opacity 0.2s !important; }
.stButton > button:hover { opacity: 0.85 !important; }
.stDownloadButton > button { background: linear-gradient(135deg, #c8b89a, #a89070) !important; color: #0d0d0d !important; font-family: 'Space Grotesk', sans-serif !important; font-weight: 700 !important; font-size: 0.9rem !important; letter-spacing: 1px !important; border: none !important; border-radius: 8px !important; width: 100% !important; padding: 0.65rem 0 !important; }
.stSelectbox label { color: #555 !important; font-size: 0.72rem !important; letter-spacing: 2px !important; text-transform: uppercase !important; }
.stFileUploader label { color: #555 !important; font-size: 0.72rem !important; letter-spacing: 2px !important; text-transform: uppercase !important; }
[data-testid="stFileUploaderDropzone"] { background: #161616 !important; border: 1px dashed #333 !important; border-radius: 10px !important; }
.stProgress > div > div { background: linear-gradient(90deg, #c8b89a, #f0ede6) !important; border-radius: 4px !important; }
...
def format_size(byte_size):
    ...
    return f"{round(mb, 2)} MB" if mb >= 1 else f"{round(byte_size / 1024, 1)} KB"
    ...
    if convert_clicked:
        ...
        for idx, upload in enumerate(uploads):
            ...
            try:
                ...
                sf.write(audio_buf, audio_data, sample_rate, format=output_format, subtype=subtype_map[output_format])
                audio_buf.seek(0)
                converted_files.append((audio_buf, output_filename))
        ...
        if converted_files:
            ...
            with zipfile.ZipFile(zip_buffer, "w") as zf:
                for audio_buf, output_filename in converted_files:
                    # ✅ Write from buffer directly into zip — no temp files needed
                    zf.writestr(output_filename, audio_buf.read())
